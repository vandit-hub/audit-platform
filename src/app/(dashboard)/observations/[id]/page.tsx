...
type Note = { id: string; text: string; actor: { email?: string | null; name?: string | null }; createdAt: string };
type Observation = {
  id: string;
  plant: Plant;
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={save} className="bg-white rounded p-4 shadow space-y-3">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" className="border px-4 py-2 rounded" onClick={submitForApproval}>Submit for approval</button>
          <button type="button" className="border px-4 py-2 rounded" onClick={() => approve(true)}>Approve (Admin)</button>
          <button type="button" className="border px-4 py-2 rounded" onClick={() => approve(false)}>Reject (Admin)</button>
          <button type="button" className="border px-4 py-2 rounded" onClick={() => publish(!o.isPublished)}>{o.isPublished ? "Unpublish" : "Publish"} (Admin)</button>
          ...
          <button type="button" className="border px-4 py-2 rounded" onClick={() => lock(["observationText","riskCategory"], true)}>Lock sample fields (Admin)</button>
        </div>
      </form>
